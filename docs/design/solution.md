...
- 目标根据机器资源预算自动限制线程和并发，默认使用整机逻辑 CPU 容量的 50% 和内存的 70%；用户可以调低、调高或完全关闭任一预算；
...
默认性能策略为 `balanced`：LocalTranscriber 的 CPU 预算默认为整机逻辑 CPU 容量的 50%，内存预算默认为 70%。两者都是可配置默认值，不是不可突破的硬上限；用户可以设为 `1–100` 的百分比，或设为 `0` 完全关闭对应预算。启用预算时不承诺操作系统采样值每一秒都精确等于配置值；FFmpeg、线程启动和系统调度可能造成短时波动。

支持以下配置：

| 配置 | 默认值 | 约束与语义 |
|---|---:|---|
| `cpu_limit_percent` | `50` | 范围 `0–100`；`0` 表示关闭 CPU 预算，`1–100` 为整机逻辑 CPU 百分比 |
| `max_workers` | `auto` | 可指定正整数，但最终值仍受 CPU/内存预算约束 |
| `threads_per_worker` | `auto` | 可指定正整数，但所有 worker 线程总和不得超过 CPU 线程预算 |
| `memory_limit_percent` | `70` | 范围 `0–100`；`0` 表示关闭配置百分比预算，但仍受当前实际可用内存约束 |
...
采用 **FunASR + SenseVoiceSmall + FSMN-VAD + CAM++** 作为主线，不以实时字幕服务为目标。前台运行是默认方式，后台运行必须由用户显式选择；两种方式都支持单文件、多文件和目录批量，并共享同一持久化任务模型、统一调度器和资源预算内的有界 worker 池。后台管理器不监听网络端口。CPU/内存预算默认分别为 50%/70%，用户可调低、调高或以 `0` 完全关闭；启用时最终并发服从 CPU、内存和用户上限，关闭内存百分比预算后仍服从当前实际可用内存。每次正式发布必须变更语义化版本、更新 `CHANGELOG.md` 并通过发布门禁；当前仍无必要为了集成而引入 MCP 或公网 HTTP 服务。