# 阶段 G：LocalTranscriber 0.2.0 发布验收
...
`0.2.0` 汇总阶段 E、F、G 的用户可见变化：事件驱动进度与动态 ETA、多文件和目录批量、显式后台模式、本机 Unix IPC 管理器、任务/批次控制与恢复、CPU/内存联合资源预算，以及版本化发布工具链。
...
升级前停止后台 worker，切换到 `v0.2.0` 后执行 `uv sync --locked`。需要回滚时停止 worker、切换回旧 tag 并重新同步依赖。`0.2.0` 未改变 canonical JSON `schema_version=1`，也未要求迁移既有任务记录。
...
2. 注释 tag `v0.2.0` 已创建并推送，且未覆盖已有 tag；
3. GitHub Release `v0.2.0` 已创建，包含更新日志、升级说明、wheel、sdist 和 SHA-256；
